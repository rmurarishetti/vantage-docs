...
```
Banking app sends push notification
  ↓
VantageNotificationListener (native Kotlin) intercepts it
  ↓
NotificationParser tries package-specific regex patterns
  ↓ match              ↓ no match
ParsedTransaction   Raw text sent to Claude Haiku
  ↓                     ↓ (structured JSON returned)
SHA-256 dedup check
  ↓
Saved to Supabase transactions table
```
