...
Your question
  ↓
Two-tier router
  ├── Regex classifier (handles ~70% of queries instantly)
  └── Claude Haiku fallback (for ambiguous queries)
  ↓
Specialist agent (Claude Sonnet)
  ↓
Tool-use loop (parallel execution)
  ├── Run relevant tools against your Supabase data
  ├── Fetch live market prices if needed
  └── Search RAG memory for context
  ↓
